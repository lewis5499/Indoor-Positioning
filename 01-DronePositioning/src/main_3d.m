...
X0=[mean(matbasestat(:,2));mean(matbasestat(:,3));mean(matbasestat(:,6));0];
...
for nepoch=1:maxepoch
    ...
    % save data
    fprintf(fop,'%s','>');
    fprintf(fop,'%9s %10s %10s %10s %10s\r\n','Time(s)','N(m)','E(m)','H(m)','b(m)');
    fprintf(fop,'%10.4f %10.4f %10.4f %10.4f %10.4f\r\n',sec(nepoch),X(1),X(2),X(3),X(4));
    fprintf(fop,"Posterior Unit Weight Error(m):%13.8f\r\n",m0);
    fprintf(fop,"Posterior Estimated Variance(m^2):\r\n");
    fprintf(fop,'%10.5f %10.5f %10.5f %10.5f\r\n',Dxx(1,:));
    fprintf(fop,'%10.5f %10.5f %10.5f %10.5f\r\n',Dxx(2,:));
    fprintf(fop,'%10.5f %10.5f %10.5f %10.5f\r\n',Dxx(3,:));
    fprintf(fop,'%10.5f %10.5f %10.5f %10.5f\r\n',Dxx(4,:));
